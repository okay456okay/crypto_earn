...
# 常见用法:
#   ./open_binance.sh -t BTC              # 开50个BTC订单
#   ./open_binance.sh -t BTC -a 10 -c 100 -p -0.0002   # 指定价格差异为-0.0002
...
# 使用getopts处理参数
while getopts "e:t:c:p:dh" opt; do
    case $opt in
        e) exchange="$OPTARG" ;;
        t) token="$OPTARG" ;;
        c) count="$OPTARG" ;;
...
if ! ps auxww|grep -q "gateio_${exchange}_hedge.py"; then
