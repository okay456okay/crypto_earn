...
该脚本用于扫描Bitget所有合约交易对，找到符合以下条件的交易对：
1. 最近30天内价格波动小于20%
...
from config import bitget_api_key, bitget_api_secret, bitget_api_passphrase, proxies, project_root
import ccxt
import time
import json
import pandas as pd
...
class BitgetContractScanner:
    ...
    def __init__(self, api_key: str = None, api_secret: str = None, api_passphrase: str = None,
        ...
        self.exchange = ccxt.bitget({
            'apiKey': api_key or bitget_api_key,
            'secret': api_secret or bitget_api_secret,
            'password': api_passphrase or bitget_api_passphrase,
        ...
        self.report_file = os.path.join(self.reports_dir, f'bitget_contract_scan_{timestamp}.json')
        self.summary_file = os.path.join(self.reports_dir, f'bitget_contract_summary_{timestamp}.txt')
        
        # 扫描参数
        self.price_volatility_threshold = price_volatility_threshold
        self.min_leverage = min_leverage
        self.days_to_analyze = days_to_analyze
        self.exchange_name = "Bitget"  # 交易所名称
        
        logger.info(f"Bitget合约扫描器初始化完成")
    ...
    def get_symbol_leverage_info(self, symbol: str) -> int:
        ...
        try:
            ...
            # 如果无法从市场信息获取，尝试API调用
            # 注意：这里可能需要根据Bitget的实际API调整
            return 100  # Bitget通常支持较高杠杆，默认返回100
    ...
    def get_funding_rate_interval(self, funding_rates_data: List[Dict]) -> float:
        ...
        if len(funding_rates_data) < 2:
            ...
            return (funding_rates_data[-1]['timestamp'] - funding_rates_data[-2]['timestamp']) / (1000 * 60 * 60)
